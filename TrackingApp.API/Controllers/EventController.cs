...
namespace TrackingApp.API.Controllers
{
    ...
    public class EventController : Controller
    {
        ...
        public async Task<IActionResult> createEvent( EventModel oneEvent)
        {
            // var EventExists = await _context.events.FindAsync(oneEvent.eventName);
            // if (EventExists != null)
            ...
            // };
            // var result = await _context.enti(newEvent);
            // if (!result.IsKeySet)
            //     return StatusCode(StatusCodes.Status500InternalServerError, new ResponseModel { Status = "Error", Message = "New Event creation failed! Please check Event Data and try again." });

            // return Ok(new ResponseModel { Status = "Success", Message = "Event created successfully!" });
            try { 

            var EventExists = await _context.events.Where(e => e.eventName == oneEvent.eventName).ToListAsync();
                if (EventExists.Count > 0)
                    return StatusCode(StatusCodes.Status500InternalServerError, new ResponseModel { Status = "Error", Message = "Event Name already exists!" });
        ...
        public async Task<IActionResult> updateEvent([FromBody] Event oneEvent)
        {
            try
            {
                ...
                EventExists.eventName = oneEvent.eventName;
                EventExists.timeFrom = oneEvent.timeFrom;
                EventExists.timeTo = oneEvent.timeTo;
                EventExists.status = oneEvent.status;
                EventExists.startFrom = oneEvent.startFrom;
                EventExists.endsOn = oneEvent.endsOn;
                EventExists.isRepeated = oneEvent.isRepeated;
                EventExists.repeatedEvery = oneEvent.repeatedEvery;
        ...
        {
            var EventExists = await _context.events.FirstOrDefaultAsync(e => e.eventId == EventID);
            if (EventExists == null)
                return StatusCode(StatusCodes.Status500InternalServerError, new ResponseModel { Status = "Error", Message = "Event Not exists!" });
        ...
        // public async Task<IActionResult> getEvent(int DateEvents)
        // {
        //     var EventExists = await _context.events.FindAsync(DateEvents);//.ToList();
